...
import { vetesDelEndpoint, vetesPatchEndpoint, vetesPatchImageEndpoint } from "../endpoints";
...
const DeleteInner =({targetId, setState}: InnerType)=>{
  const handleDelete = async () => {
     const ok = await deleteRegis(vetesDelEndpoint, targetId);
     if (ok) {
...
const RenovInner =({targetId, setState}: InnerType)=>{
  ...
  const handleRenov = async (e: React.FormEvent) => {
    e.preventDefault();
     ...
     const ok = await renovRegis(vetesPatchEndpoint, targetId, newdate);
     if (ok) {
...
const ImageEditInner =({targetId,setState}: InnerType)=>{
  ...
  const handleReimage = async (e :React.FormEvent) => {
    e.preventDefault()
    if(!file) return
     const ok = await reimageRegis(vetesPatchImageEndpoint, targetId, file);
     if (ok) {
      alert("Renovacion exitosa")
       setState("exito"); // actualiza la lista en el estado del padre
     }
...
const EstablishmentRow: React.FC<EstablishmentRowProps> = ({ prof, setSelectedProf, setShowModal, setmodalType }) => {
  const hoy = new Date();
