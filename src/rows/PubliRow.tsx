...
import { publiDelEndpoint, publisPatchEndpoint, publisPatchImageEndpoint } from "../endpoints";
...
const DeleteInner =({targetId, setState}: InnerType)=>{
  const handleDelete = async () => {
     const ok = await deleteRegis(publiDelEndpoint, targetId);
     if (ok) {
...
const RenovInner =({targetId, setState}: InnerType)=>{
  ...
  const handleRenov = async (e :React.FormEvent) => {
    e.preventDefault()
     const ok = await renovRegis(publisPatchEndpoint, targetId, newdate);
     if (ok) {
...
const ImageEditInner =({targetId,setState}: InnerType)=>{
  ...
  const handleReimage = async (e :React.FormEvent) => {
    e.preventDefault()
    if(!file) return
     const ok = await reimageRegis(publisPatchImageEndpoint, targetId, file);
     if (ok) {
      alert("Renovacion exitosa")
       setState("exito"); // actualiza la lista en el estado del padre
     }
...
const PubliRow: React.FC<PubliRowProps> = ({ prof, setSelectedProf, setShowModal, setmodalType }) => {
  const hoy = new Date();
